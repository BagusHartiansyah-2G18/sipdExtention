	...
	setTimeout(function(){
		...
		if(title_admin.length >= 1){
			...
			// Data RPJMD
				...
				jQuery('#gdata').on('click', function(){
					gdata();					
				});
			...
			// Renja Belanja
				...
				if(
					...
					singkron_rka += ''
						...
						+'</button>'
						+'<button style="margin-left: 20px;" class="btn btn-sm btn-warning" id="gdata">gdata</button>';
...
function cekUrlBGS(current_url, nomor=1){
	getToken();
	...
	setTimeout(function(){
		...
		if(title_admin.length >= 1){
			...
			const cbAkction = `<select id="cbAkctionMFC" class="btn btn-success btn-sm">
				<option value="" >None Action</option>
				<option value="allSubKegiatanPublik" >All Sub Kegiatan Publik</option>
				<option value="subKegiatanSKPD">SubKegiatanSKPD</option>
				<option value="SKPD">SKPD</option>
				<option value="ssh">Standar Harga</option>
			</select>`;
			// jQuery('.aksi-extension').remove();
			if(jQuery('#cbAkctionMFC').length == 0){
				title_admin.closest('.menu-item').after(cbAkction);
			}

			jQuery('#cbAkctionMFC').on('change', function(){
				switch ($(this).val()) {
					case "allSubKegiatanPublik":
						return allSubKegiatanPublik();
					case "subKegiatanSKPD":
						return subKegiatanSKPD(2);
					case "SKPD":
